...
export function ExperimentStatusProvider({ children }) {
    ...
    const checkExperimentStatus = async (sessionId) => {
        try {
            ...
            if (!data.experiment) {
                navigate('/experiment-error', { state: { status: 'not_found' } });
                return false;
            }
            ...
            switch (data.experiment.status) {
                ...
                case 'pause':
                    ...
                    navigate('/experiment-error', { state: { status: 'pause' } });
                    return false;
                ...
                case 'stop':
                    ...
                    navigate('/experiment-error', { state: { status: 'stop' } });
                    return false;
...
export const useExperimentStatus = () => {
    ...
    if (!context) {
        throw new Error('useExperimentStatus must be used within an ExperimentStatusProvider');
    }
    return context;
};
